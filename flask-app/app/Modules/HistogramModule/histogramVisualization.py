...
def CreateFigure(_algorithm, _data, _energie, _signalType, _receivedValue, _iterations,
                                 _originRadius, _distanceToOrigin, _receiverNumber, _senderNumber, 
                                 _radius, _formation, _b, _pathLoss, _waveLenght, _min, _max, _top):
    ...
    #n, bins, patches = ax.hist(_distanceToOrigin, 200, weights=np.ones(_receiverNumber)) #for random positions

    n, bins, patches = ax.hist(Z3, 100, weights=np.ones(iterations) / iterations, range=(_min,_max))

    #n, bins, patches = ax.hist(Z3, 100, density=True, stacked=True, range=(2500,5000))
    ...
    ax.set_xlabel('#Receiver received Minimum Signal Value')

    #x.set_title( '{0}, #R = {1}, #S = {2}, Samples = {3}, {4}'.format(_algorithm, _receiverNumber, _senderNumber, iterations, _formation) + "\n"+ r'$\lambda = {0}$'.format(_waveLenght) 
                    #+ ' loss = {0}, b = {1}, radius = {2}, signal value = {3}'.format(_pathLoss, _b, _radius, _receivedValue))

    ax.set_title( 'Mean = {0}, Varianz = {1}'.format(mu, sigma))
