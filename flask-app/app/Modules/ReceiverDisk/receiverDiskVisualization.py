...
def InitFigureScatter3dNoBar(ax, fig, _Z ,_model, _receiverPos, _radius, _orientation, _vmin):
    Z = _Z
    if(session['cutOrigin_RD']):
        originRadius = session['originRadius_RD']
        distanceOrigin = np.sqrt(np.square(_model[0]) + np.square(_model[1]))
        Z = np.where(distanceOrigin <= originRadius, -1, Z)
    # if(_vmin != None):
    #     Z = np.where((Z <= _vmin), None, Z)
    #Z = np.where(_model[3] > _radius, None, Z)
    Z = np.where(Z < _vmin, None, Z)
    XX,YY = np.where(Z == None, None, (_model[0], _model[1]))
    if _orientation == 'Horizontal':
        cs = ax.scatter(XX, YY,_receiverPos[2], c='black', s=0.1, alpha=1)
    else:
        cs = ax.scatter(XX, YY,_receiverPos[1], c='black', s=0.1, zdir='y', alpha=1)
    ax.set_zlim(_receiverPos[2] - _radius, _receiverPos[2] + _radius)
    ax.set_ylim(_receiverPos[1] - _radius, _receiverPos[1] + _radius)
    ax.set_xlim(_receiverPos[0] - _radius, _receiverPos[0] + _radius)
    ax.set_xlabel('X')
...
def InitFigureScatter2dNoBar(ax, fig, _Z,_model, _receiverPos, _radius, _orientation, _vmin):
    Z = _Z
    #Z = np.where(_model[3] > _radius, None, Z)
    ...
    XX,YY = np.where(Z == None, None, (_model[0], _model[1]))
    if _orientation == 'Horizontal':
        cs = ax.scatter(XX, YY,  s=0.1, c='Black', alpha=1, vmin=_vmin)
        ax.set_xlabel('X')
        ax.set_ylabel('Y')
        ax.set_ylim(_receiverPos[1] - _radius, _receiverPos[1] + _radius)
        ax.set_xlim(_receiverPos[0] - _radius, _receiverPos[0] + _radius)
...
def InitFigure(ax, fig, Z,_model, _colors, _receiverPos, _radius, _orientation, _vmin, _vmax, _plotTyp, _dimension, _logScale):
    ...
    if(_vmin == _vmax):
        if(_dimension == '3d'):
            InitFigureScatter3dNoBar(ax, fig, Z , _model, _receiverPos, _radius, _orientation, _vmin)
        else:
            InitFigureScatter2dNoBar(ax, fig, Z , _model, _receiverPos, _radius, _orientation, _vmin)
    else:
        if(_dimension == '3d'):
