...
def Visualize(_model, _colors, _receiverPos, _wavelength,_pathLoss,_b, _dimension, _plotTyp, _debug = False):
    ...
    if(_plotTyp == 'phase'):
        Z = (np.angle(_model[2]) + 2 * np.pi) % (2 * np.pi)
    ...
    if(_dimension == '3d'):
        ...
        cs = ax.scatter(XX, YY,0, c=Z, s=0.1,cmap=plt.get_cmap(_colors), alpha=1)
        ax.set_zlabel('Z')
        ax.set_zlim(-1, 1)
    else:
    ...
    if(_plotTyp == 'phase'):
        cbar = fig.colorbar(cs, ticks=[0.0001, 1/2 * cmath.pi ,cmath.pi, 3/2 * cmath.pi ,2 * cmath.pi - 0.0001])
    else:
        fig.colorbar(cs, ax=ax)       
    if(_plotTyp == 'phase'):
        cbar.ax.set_yticklabels([0, r'$\frac{1}{2} \pi$' , r'$\pi$',r'$\frac{3}{2} \pi$',r'$2\pi$']) 
