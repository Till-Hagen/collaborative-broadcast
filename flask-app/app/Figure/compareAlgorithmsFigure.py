...
def CreateCompareAlgorithmsFigure(_algorithms, _data, _energie, _signalType, _minReceivedValue, _range, _originRadius, _distanceToOrigin):
    ...
    energie = np.sum(_energie, axis=1, dtype=int) / iterations
    ...
    for i in range(0,len(_algorithms)):
        ...
        for s in singalValues:
            ...
            res.append(np.sum(Z2) / iterations)
    ...
    legend1 = ax.legend(energie, bbox_to_anchor=(0., 1.02, 1., .102), loc='lower left',title="Used energy sum(a)",
...
def CreateCompareAlgorithmsFigure2(_algorithms, _data, _energie, _signalType, _minReceivedValue, _range, _originRadius, _distanceToOrigin):
    ...
    Z1 = np.where(_distanceToOrigin[i] <= _originRadius, 0, Z)

    Z2 = np.sum(Z1, axis=1)

    n, bins, patches = ax.hist(Z2, 200, density=1)
